...
const server = http.createServer((req, res) => {
	try {
		...
		if (decodedUrl.startsWith("/category") || decodedUrl.startsWith("/tag") || decodedUrl.startsWith("/search")) {
		...
		fs.readFile(filePath, (err, data) => {
			if (err) {
				if (err.code === "ENOENT") {
					data = fs.readFileSync(path.join(baseDirectory, "404.html"));
					res.writeHead(404, {
...
fs.watch(path.join(__dirname, ".."), { recursive: true }, (eventType, filename) => {
	if (!generating && filename && !filename.includes("dist") && !filename.includes(".git") && !filename.includes("node_modules") && !filename.includes(".DS_Store")) build(filename);
});
