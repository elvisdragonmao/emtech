...
const slugify = text =>
	text
		...
		.replace(/[^\p{L}\p{N}\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF\-_]+/gu, "") // Remove all non-word characters except hyphens/underscores
...
const initDist = async () => {
	...
	await Promise.all(["dist/static", "dist/p/clean", "dist/p/meta", "dist/meta/tag", "dist/meta/category"].map(path => fs.mkdir(path, { recursive: true })));
};

const replacePlaceholders = (template, replacements) => Object.keys(replacements).reduce((str, key) => str.replaceAll(`{{${key}}}`, replacements[key]), template);
...
const generatePartials = async () => {
	...
	while (partialsToRender.size)
		for (const partial of [...partialsToRender]) {
			...
			if (rendered !== partialsContent[partial]) partialsContent[partial] = rendered;
	...
	await Promise.all([fs.copyFile("dist/home/index.html", "dist/index.html"), fs.copyFile("dist/post/index.html", "dist/p/index.html"), fs.copyFile("dist/404/index.html", "dist/404.html")]);
	await Promise.all([fs.rm("dist/home", { recursive: true }), fs.rm("dist/post", { recursive: true }), fs.rm("dist/404", { recursive: true })]);
};
...
const copyStatic = async () => {
	const copyFilteredFiles = async () => {
		...
		const filteredFiles = files.filter(file => file.isFile() && file.name !== "index.md");
		const copyPromises = filteredFiles.map(async file => {
			const sourcePath = path.join(file.parentPath, file.name);
			const targetPath = path.join(targetDir, file.parentPath.split("\\").pop().split("/").pop(), file.name);
			await fs.cp(sourcePath, targetPath, { recursive: true });
			// test if is image
			if (/\.(jpe?g|png|gif|bmp|webp|svg)$/i.test(file.name) && !imageMeta[file.parentPath]) {
				const { width, height } = await sharp(sourcePath).metadata();
				imageMeta["/static/" + file.parentPath.split("\\").pop().split("/").pop() + "/" + file.name] = `width="${width}" height="${height}"`;
			}
		});
	...
	await Promise.all([copyFilteredFiles(), fs.cp("static", "dist/static", { recursive: true }), fs.cp("public", "dist", { recursive: true })]);
};
...
async function processPosts() {
	...
	const postFolders = items.filter(item => item.isDirectory()).map(dir => dir.name);
	...
	await Promise.all(
		postFolders.map(async postID => {
			try {
				...
				let markdownContent = (await fs.readFile(markdownFile, "utf8")).replace(/<!--[\s\S]+?-->/g, "");
				...
				let thumbnail = postMeta.thumbnail || (imageMeta[`/static/${postID}/thumbnail.webp`] ? `/static/${postID}/thumbnail.webp` : "");
				if (!postMeta.colors && thumbnail.includes(".") && !thumbnail.includes("http")) {
					colors = await findRepresentativeColors(path.join("dist", thumbnail));
					postMeta.colors = "linear-gradient(135deg, " + colors[0].join(", ") + ")";
					postMeta.color = colors[0][1];
					postMeta.thumbnailSize = colors[1];
				}

				const chineseCharCount = (markdownContent.match(/[\u4e00-\u9fa5]/g) || []).length;
				const englishWordCount = (markdownContent.match(/\b\w+\b/g) || []).length;
				const length = chineseCharCount + englishWordCount;
				// turn to k, if length > 1000. Fixed to 1 decimal place
				postMeta.length = length > 1000 ? (length / 1000).toFixed(1) + "k" : length;
				// postMeta.lastmod =
				...
				postMeta = {
					...
					htmlContent: (new Date() - new Date(postMeta.date)) / (1000 * 60 * 60 * 24) < 30 ? htmlContent : ""
				};

				postsMeta.push(postMeta);
				//<div class="header-categorie">閒聊</div> for each, combine all to a string
				const headerCategories = postMeta.categories ? postMeta.categories.map(category => `<a href="/category/${category}"><div class="header-categorie">${category}</div></a>`).join("") : "";
				const headerTags = postMeta.tags ? postMeta.tags.map(tag => `<a href="/tag/${tag}"><div class="header-tag">${tag}</div></a>`).join("") : "";
				const postTags = postMeta.tags ? postMeta.tags.map(tag => `<a href="/tag/${tag}"><div class="post-tag">${tag}</div></a>`).join("") : "";
				...
				const replacements = {
					...
					BreadcrumbList,
					thumbnail,
					thumbnailWidth: postMeta.thumbnailSize ? postMeta.thumbnailSize[0] : "",
					thumbnailHeight: postMeta.thumbnailSize ? postMeta.thumbnailSize[1] : "",
				...
				await fs.writeFile(`dist/p/${postID}/index.html`, fullPostPageHtml);
			} catch (error) {
	...
	await Promise.all(postsMeta.map(post => fs.writeFile(`dist/p/meta/${post.id}.json`, JSON.stringify(post))));
	...
	for (const category in categories) {
		if (config.category[category]) {
			categories[category].description = config.category[category].description;
		}
	}

	const writePromises = [
		fs.writeFile("dist/meta/latest.json", JSON.stringify(postsMeta.slice(0, 10))),
		fs.writeFile("dist/meta/search.json", JSON.stringify(search)),
		...Object.entries(tagsMap).map(([tag, posts]) => fs.writeFile(`dist/meta/tag/${tag}.json`, JSON.stringify(posts))),
		...Object.entries(categoriesMap).map(([category, posts]) => fs.writeFile(`dist/meta/category/${category}.json`, JSON.stringify(posts))),
		fs.writeFile("dist/meta/tags.json", JSON.stringify({ tags, categories }))
	];
...
function extractFrontMatter(content) {
	...
	if (frontMatterMatch) {
		...
		const lines = frontMatter.replaceAll("  ", " ").split("\n").slice(1, -1); // 去掉 '---'
		meta = {};
...
const generateSitemapAndRSS = async () => {
	...
	const allPage = ["https://emtech.cc", "https://emtech.cc/random", "https://emtech.cc/rss.xml", "https://emtech.cc/sitemap.xml"]
	...
	await fs.writeFile("dist/pages.txt", allPage.map(url => encodeURI(url)).join("\n"));
...
async function findRepresentativeColors(imagePath) {
	...
	const colors = await Promise.all(
		regions.map(async region => {
			const { data } = await sharp(imagePath).extract(region).raw().ensureAlpha().toBuffer({ resolveWithObject: true });
			...
			for (let i = 0; i < sampleSize; i++) {
				const idx = i * 4;
				const color = "#" + [0, 1, 2].map(j => data[idx + j].toString(16).padStart(2, "0")).join("");
