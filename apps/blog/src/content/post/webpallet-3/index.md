...
{{youtube id="WrNpToBObo0" title="網棧 EP.3 CSS 基礎教學影片"}}

影片替代內容：這支影片介紹 CSS 基礎，包含選取元素、設定文字與區塊樣式，以及用 CSS 裝飾網頁。本文已提供同等的文字教學與範例，可直接閱讀下方內容。
...
<img src="https://developer.mozilla.org/en-US/docs/Web/CSS/gradient/linear-gradient/linear-gradient.png" alt="CSS 線性漸層範例" style="width:300px;display:inline;" />
...
- `:active`：當元素被激活時應用的樣式。通常用於按下按鈕或連結時，以顯示按下效果。
- `:visited`：應用於已存取過的連結的樣式。這使得存取過的連結可以與未存取的連結區分開來。
