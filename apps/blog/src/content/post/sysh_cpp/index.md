...
當然如果你只是想看解答<s>然後像月同學一樣手機開超大聲打音遊</s>可以<a href="#題目解答">跳到題目解答</a>

> [!NOTE] C 語言
>
> 如果你忘記 C 語言的語法想看看可以閱讀[C 語言 APCS 筆記](https://emtech.cc/p/APCS_note/)，不過相信你學過 C++ 就不會想要碰它了。
...
> [!NOTE] 提示筆記
>
> 這裡使用了`for-each`循環。它將`y`設定為`a`的第一個元素，運行大掛號（可省略）的程式，然後它將`y`設定為`a`的第二個元素，然後...
>
