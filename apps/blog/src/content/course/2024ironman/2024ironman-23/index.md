...
> 今日範例程式：[開啟第 23 天範例程式](https://github.com/elvisdragonmao/2024-GitHub-Actions/tree/main/23)

## 設定 Node.js 專案

在開始設計 CI/CD 流程之前，我們需要確保已經在 GitHub 上建立了一個 Node.js 專案，並在本地完成了基本的項目配置。你可以使用現有的專案，或是依照以下步驟簡單配置一個。
...
在項目的根目錄下建立一個 `.github/workflows` 文件夾，並在其中建立一個名為 `ci.yml` 的文件。
...
你可以在 CI 過程中加入 ESLint 或 Prettier 來自動驗證程式風格。
...
設計一個穩定的 CI/CD 流程對於保證軟體的品質和部署效率都至關重要。本文介紹了如何使用 GitHub Actions 搭建 Node.js 的多階段 CI/CD 流程，並涵蓋了構建、測試、部署和發布的自動化。通過實踐這些步驟和技巧，你可以更輕鬆地管理你的 Node.js 專案，並確保其在不同環境中的穩定性。
